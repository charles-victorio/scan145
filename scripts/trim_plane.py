"""
Slice a GLB flat along any axis (X/Y/Z) at a world-space cutoff, discarding
one side and cutting straddling triangles so the boundary is a clean, exact
edge (not a jagged stair-step from whole-triangle removal). Writes a new
file; the source is left untouched.

Generalized version of trim_height.py (which is Y-axis-only, "keep below" —
kept as-is since main.ts's comments reference it by name for the original
ceiling cut). Same technique: only appends new accessors/bufferViews
(interpolated position+UV for the cut triangles, plus the filtered index
buffers) to the end of the binary blob — materials and embedded images are
preserved byte-for-byte. Old, now-unused accessors/bufferViews are left in
place as harmless dead data rather than attempting a full buffer repack.

Usage:
    python3 trim_plane.py <src.glb> <dst.glb> <axis:x|y|z> <cutoff> <keep:above|below>

"above" keeps values >= cutoff (discards < cutoff); "below" keeps values
<= cutoff (discards > cutoff).

Example — keep everything with z >= -3.05 (cut away the z < -3.05 tail):
    python3 trim_plane.py scan.glb scan.out.glb z -3.05 above
...
def clip_triangle(pos3: np.ndarray, uv3: np.ndarray, axis: int, cutoff: float, keep_ge: bool):
    # Sutherland-Hodgman clip of one triangle against a half-space on `axis`.
    # Preserves winding order; returns a convex polygon (0, 3, or 4 verts).
    vals = pos3[:, axis]
    inside = (vals >= cutoff) if keep_ge else (vals <= cutoff)
    out_pos, out_uv = [], []
    for i in range(3):
        ...
        if curr_in:
            if not prev_in:
                t = (cutoff - prev_p[axis]) / (curr_p[axis] - prev_p[axis])
...
def main():
    src, dst, axis_name, cutoff_s, keep = sys.argv[1:6]
    axis = AXES[axis_name.lower()]
    cutoff = float(cutoff_s)
    keep_ge = keep.lower() == "above"
    ...
    for mesh in gltf.meshes:
        for prim in mesh.primitives:
            ...
            tri_vals = positions[tris][:, :, axis]
            if keep_ge:
                all_in = (tri_vals >= cutoff).all(axis=1)
                all_out = (tri_vals < cutoff).all(axis=1)
            else:
                all_in = (tri_vals <= cutoff).all(axis=1)
                all_out = (tri_vals > cutoff).all(axis=1)
            ...
            for tri in tris[straddling]:
                poly_pos, poly_uv = clip_triangle(positions[tri], uvs[tri], axis, cutoff, keep_ge)
                if len(poly_pos) < 3:
                    continue
